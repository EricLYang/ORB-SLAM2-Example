...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    // for every kp in last image
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        cv::KeyPoint kp1 = F1.mvKeysUn[i1];
        int level1 = kp1.octave; //!< octave (pyramid layer) from which the keypoint has been extracted
        if(level1>0)    // ???
            continue;
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            if(dist<bestDist)   // best matcher
            {
                bestDist2=bestDist;
                bestDist=dist;
                bestIdx2=i2;
            }
            else if(dist<bestDist2) // 2th best matcher
            {
                bestDist2=dist;
            }
        }
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                if(vnMatches21[bestIdx2]>=0)    // matched before, clear
                {
