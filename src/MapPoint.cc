...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    if(pKF->mvuRight[idx]>=0)   // 双目或者RGB-D
        nObs+=2;
    else
        nObs++; // 单目
}
...
void MapPoint::ComputeDistinctiveDescriptors()
{
...

    {
        ...
        if(mbBad)
            return;
        observations=mObservations;// std::map<KeyFrame*,size_t> mObservations; //观测到该MapPoint的KF和该MapPoint在KF中的索引
    }
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    cv::Mat Pos;
    {
        ...
        Pos = mWorldPos.clone();    // 3d点在世界坐标系中的位置 Pw
    }
