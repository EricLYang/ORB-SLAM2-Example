 ...
 * RANSAC achieves its goal by repeating the following steps:
    1. Select a random subset of the original data. Call this subset the hypothetical inliers(假设集).
    2. A model is fitted to the set of hypothetical inliers.
    3. All other data are then tested against the fitted model. Those points that fit the estimated model well,
       according to some model-specific loss function, are considered as part of the consensus set(共识).
                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
    {
        if(vMatches12[i]>=0)
        {
            mvMatches12.push_back(make_pair(i,vMatches12[i]));  // pair<int, int>
            mvbMatched1[i]=true;
        }
        else
            mvbMatched1[i]=false;
    ...
    // ref是引用的功能:http://en.cppreference.com/w/cpp/utility/functional/ref (ref = &)
    // 计算homograpy并打分
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    cv::Mat T1, T2;
    Normalize(mvKeys1,vPn1, T1);    // [x' y' 1]T = T * [x y 1]T
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 恢复原始的均值和尺度
        // P2' = H21'*P1' => T2*P2 = H21*(T1*P1) => H21 = T2-1*H21'*T1
        H21i = T2inv*Hn*T1;
        H12i = H21i.inv();
...
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
    ...
    return vt.row(8).reshape(0, 3); // v的最后一列 => cv::Mat (3*3)
}
// p = [u,v,1]T
// p'Fp = 0 整理可得：Af = 0
 ...
 * @param  vP2 归一化后的点, in current frame
 * @return     基础矩阵
 * @see        Multiple View Geometry in Computer Vision - Algorithm 11.1 p282(king: P297) (中文版 p191)
 */
cv::Mat Initializer::ComputeF21(const vector<cv::Point2f> &vP1,const vector<cv::Point2f> &vP2)
{
    ...
    w.at<float>(2)=0; // 秩2约束，将第3个奇异值设为0    // (c, c, 0)

    return  u*cv::Mat::diag(w)*vt;  // F = U*W*VT
}
 ...
 *
 * @see
 * - Author's paper(ORB-SLAM1) - IV. AUTOMATIC MAP INITIALIZATION （2）
...
float Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        //s |v1| = |h21inv h22inv h23inv||v2|
        // |1 |   |h31inv h32inv h33inv||1 |
        const float w2in1inv = 1.0/(h31inv*u2+h32inv*v2+h33inv);    // scale para: s
