                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));    // cv => Eigen => g2o::SE3Quat(R,t);
        vSE3->setId(pKF->mnId);
        vSE3->setFixed(pKF->mnId==0);   // KF id0 fixed
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        const int id = pMP->mnId+maxKFid+1; // After T vertex
        vPoint->setId(id);
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        else
        {
            ...
            pKF->mnBAGlobalForKF = nLoopKF; // 关键帧的个数
        }
    }
...
/**
 * @brief Pose Only Optimization    // 开始优化，总共优化四次
 *
 * 3D-2D 最小化重投影误差 e = (u,v) - project(Tcw*Pw) \n
 ...
 * 1. Vertex: g2o::VertexSE3Expmap()，即当前帧的Tcw
 * 2. Edge:
 *     - g2o::EdgeSE3ProjectXYZOnlyPose()，BaseUnaryEdge     // Case Mono cam
 *         + Vertex：待优化当前帧的Tcw
 *         + measurement：MapPoint在当前帧中的二维位置(u,v)
 *         + InfoMatrix: invSigma2(与特征点所在的尺度有关)
 *     - g2o::EdgeStereoSE3ProjectXYZOnlyPose()，BaseUnaryEdge   // Case Stereo cam
 *         + Vertex：待优化当前帧的Tcw
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // Set MapPoint vertices
    const int N = pFrame->N;    // KeyPoints数量
    ...
    // 步骤3：添加一元边Edge：相机投影模型 // g2o::EdgeSE3ProjectXYZOnlyPose
    {
        unique_lock<mutex> lock(MapPoint::mGlobalMutex);
        // king: for every KPoint
        for(int i=0; i<N; i++)
        {
            ...
            if(pMP)
            {
                ...
                else  // Stereo observation 双目
                {
                    ...
                    const cv::KeyPoint &kpUn = pFrame->mvKeysUn[i];
                    const float &kp_ur = pFrame->mvuRight[i];   // right image x(u) axis
                    obs << kpUn.pt.x, kpUn.pt.y, kp_ur;// 这里和单目不同

                    g2o::EdgeStereoSE3ProjectXYZOnlyPose* e = new g2o::EdgeStereoSE3ProjectXYZOnlyPose();// 这里和单目不同(Stereo)
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        optimizer.optimize(its[it]);    // optimize
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            if(it==2)
                e->setRobustKernel(0); // 除了前两次优化需要RobustKernel以外, 其余的优化都不需要 king: ??
        }
        // Stereo cam
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            if(it==2)   // 除了前两次优化需要RobustKernel以外, 其余的优化都不需要 king: ??
                e->setRobustKernel(0);
        }
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    // Set Local KeyFrame vertices  // king: Tl
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));
        int id = pMP->mnId+maxKFid+1;   // king: After KFs
        vPoint->setId(id);
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                else // Stereo observation
                {
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                    e->setMeasurement(obs); // king: (u,v,ur)
                    const float &invSigma2 = pKFi->mvInvLevelSigma2[kpUn.octave];
                    Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2;
                    e->setInformation(Info);
