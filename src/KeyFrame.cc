...
void KeyFrame::ComputeBoW()
{
    ...
    DBoW2::BowVector mBowVec; ///< Vector of words to represent images // std::map<WordId, WordValue>

    // std::map<NodeId, std::vector<unsigned int> > // feature vector of nodes and feature indexes
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    cv::Mat Rwc = Rcw.t();
    Ow = -Rwc*tcw;  // t from cam to world
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    vPairs.reserve(mConnectedKeyFrameWeights.size());   // std::map<KeyFrame*,int> mConnectedKeyFrameWeights;
    ...
    for(size_t i=0, iend=vPairs.size(); i<iend;i++)
    {
        ...
        lWs.push_front(vPairs[i].first);    // 权重从大到小
    }
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    // wubo wrong!!! see offer book code vectorBound.cpp
...
void KeyFrame::UpdateConnections()
{
    ...
    //===============1==================================
    map<KeyFrame*,int> KFcounter; // 关键帧-权重(KF-cnt)，权重为其它关键帧与当前关键帧共视3d点的个数
    ...
    // pair<int,KeyFrame*>将关键帧的权重写在前面，关键帧写在后面方便后面排序(std::sort)
    vector<pair<int,KeyFrame*> > vPairs;
